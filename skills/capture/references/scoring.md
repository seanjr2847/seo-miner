...
| intent_split | 한 페이지가 검색 의도 둘을 떠안음 — cannibalization 의 거울이다(저쪽은 한 검색어를 여러 페이지가 나눠 갖는 것, 이쪽은 한 페이지가 여러 의도를 떠안는 것). 할 일은 고치기가 아니라 **가르기** | `scoring.intent_split()` — 최신 스냅샷(`snapshot_pair()`)에서 검색어마다 노출 1등 페이지 하나로 묶고(brief 의 페이지 규칙과 같다), `query_intent()`(자리 판정은 프로젝트 locale — `site_words_of()`)로 의도를 매겨 `intent_share()` 로 접는다. 1·2위 의도가 **둘 다 명시 의도**여야 하고(`INTENT_DEFAULT` 가 끼면 제외), 2위 의도 노출 ≥ `SPLIT_MIN_SECOND_IMP`(=20) · 2위 의도 검색어 ≥ `SPLIT_MIN_SECOND_QUERIES`(=2) · 페이지 전체 노출 ≥ `SPLIT_MIN_IMP`(=50), 홈·언어 루트는 제외(`is_site_root()`). 2위 의도 노출 내림차순. target 은 URL 이고, 점수에 쓰는 노출은 **2위 의도 몫만**이다(전체를 쓰면 잘 걸리는 묶음까지 얹혀 늘 위로 선다). 요청문 꼴은 `brief.KIND_SHAPE` 의 `split_page` |
