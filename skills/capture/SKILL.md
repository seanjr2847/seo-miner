...
### /capture gsc {P}
경로는 하나다 — **서비스 계정 연동(필수)**: `python scripts/collect_gsc.py --project {P}`.
키 1개(`~/.capture/gsc_service_account.json`)로 모든 사이트를 자동 수집한다
(query·page 차원, 행 제한 사실상 없음). 아직 안 붙어 있으면 수집을 시도하지 말고
setup 스킬의 "GSC 연결(서비스 계정)" 절차대로 콘솔 클릭을 내가 대신 해준다
(무료, 계정당 1회 5분). 절차를 사용자에게 읽어주지 말 것.
