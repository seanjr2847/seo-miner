...
5. **루프 클로즈.** 작업 완료 시 `scripts/createdb.py done`으로 Brain에 기록해
   ...
   **이 스킬을 안 거치고 손으로 고친 것도 똑같이 기록한다.** 안 그러면 Brain은
   영영 모른다 — 다음 런이 같은 기회를 또 제안하고, 대시보드는 "실행 0건"이라고
   말한다. (aitierlist 첫 실사용에서 상위 5건이 전부 이 상태였다: 리포에는
   `9f5ba4c`·`8647c5f` 같은 커밋으로 이미 반영돼 있는데 기회는 전부 `new`.)
...
1-b. **쓰기 전에 리포와 대조한다.** 기회 대상 파일의 `git log`(그리고 그 파일의
   ...
   실행: <커밋>"`으로 닫고 목록에서 뺀다. 이 대조 없이 쓰면 남이 방금 손본
   문구를 덮어쓴다 — 실사용에서 상위 5건이 전부 이미 반영된 상태였다.
