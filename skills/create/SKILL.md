...
5. **루프 클로즈.** 작업 완료 시 `scripts/createdb.py done`으로 Brain에 기록해
   다음 capture 런이 이 콘텐츠의 성과를 측정하게 한다. 기록하면 기회는 **진행 중**
   (acked)에 머문다 — 글 하나로 묶음 기회(pSEO·콘텐츠 갭)가 닫히면 안 된다.
   완료(done)가 되는 길은 셋이다: 사람이 대시보드에서 누르거나, `createdb.py sync` 가
   그 작업의 **PR 이 머지된 것**을 gh 로 확인하거나(머지 = 사람이 한 발행), capture 가
   새 데이터로 조건이 풀린 것을 확인했는데 그 기회에 작업 기록이 있을 때. 작업 기록
   없이 풀린 것은 '저절로 풀림'(resolved)이라 완료 후 관찰(우리 효과)에 안 들어간다.
...
3. 결과를 `$CAPTURE_HOME/projects/{P}.repo.yaml`로 저장하고 사용자 검수를 받는다.
   ...
   **리포 경로는 yaml 에 적지 않는다** — `python ../capture/scripts/paths.py dir {P} <리포 절대경로>`
   로 사이트별 로컬 폴더 장부(`$CAPTURE_HOME/dirs.json`, 대시보드 [설정]의 폴더 표)에
   적는다. "이 폴더가 어느 사이트냐"(`db.repo_project()`)와 [열기] 버튼이 그 한 벌만
   읽는다. yaml 에 따로 적으면 두 벌이 되고 한쪽만 낡는다.
...
1-b. **쓰기 전에 리포와 대조.** plan 시작 시 `python scripts/createdb.py sync {P} --repo .`
   를 먼저 실행해 커밋 메시지 `[opp #id]` 규약으로 이미 반영된 기회를 진행 중으로
   옮기고 기록한다. 같은 명령이 이어서 **머지 반영**도 한다 — 머지를 아직 모르는 작업
   기록(브랜치가 있는 것)마다 `gh pr list --head <브랜치> --state merged` 로 묻고,
   머지됐으면 기록을 병합됨으로, 그 기회를 완료로 옮긴다(완료 시각 = 머지 시각).
   gh 가 없거나 로그인이 안 됐으면 한 줄 알리고 건너뛴다. 이 대조 없이 쓰면 남이
   방금 손본 문구를 덮어쓴다.
...
### /create status {P}
먼저 `createdb.py sync {P} --repo .` 로 머지를 반영한 뒤 `createdb.py list {P}` 로
작성 이력·머지 여부를 보여준다. gh 를 못 쓰는 환경이면 머지된 건을
`createdb.py merged <creation_id>` 로 손으로 갱신한다(이건 기회 상태는 안 바꾼다).
웹에 등록한 사이트는 기회만 완료로 옮겨지고 기록의 '병합 전' 표시는 그대로다 —
서버에 병합 표시 창구가 아직 없다.
