...
서버(즉석 조회)와 `collect_gsc.py`(벌크 수집)가 같이 쓴다. 인증은 한 벌이라
OAuth로 붙인 경우엔 서비스 계정 없이 그 토큰을 벌크 수집도 그대로 쓴다
(`collect_gsc.py`가 MCP 서버의 인증 해석기를 빌린다 — setup.md 4-B). 사이트가 늘어도 이
