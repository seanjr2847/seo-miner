...
**화면으로 보여줄 때**: `python scripts/doctor.py --web` — 로컬 대시보드를 띄우고
상단 배너에 같은 진단(요약·다음 한 걸음·할 일)이 뜬다. 사용자가 "화면으로/대시보드로
보여줘"라고 하면 이걸 백그라운드로 실행한다. 내 판단용 데이터는 계속 CLI/`--json`을 읽는다.
