...
호스팅 화면(`/d`)은 OAuth 때문에 그냥 못 띄운다. `test_render.py` 의 `serve_hosted()` 가
**실제 FastAPI 앱**(`server.app.app`)을 로컬에 띄운다 — 그걸 쓴다. 로그인은
`_require_uid` 의존자만 갈아 끼운다. stdlib `serve()` 로 호스팅 화면을 띄우면 안 된다:
호스팅 전용 `/api/*` 가 없어 404 이고, 인증·타입 검증을 안 거쳐서 운영에서 422·500 인
호출이 초록으로 지나간다 — 동시 요청 500 이 그렇게 이틀 동안 운영에 있었다.
...
12. 요청문의 꼴은 `brief.py` 의 `SHAPE_NAMES` 한 벌이다(이름·개수는 거기서 본다 —
    여기 나열하면 그게 사본이 된다) — 기회는 서버가 쓴 `o.brief` 를 그대로 그리고, 기회로
    안 올라온 행의 폴백 `askBlock` 은 그 이름만 넘긴다. 셸에 옛 틀(규칙 문장·
    진단별 산출물 사본)이 남으면 두 벌이다
13. 묶음(메뉴 제목 = 다시 재는 단위)은 `run_all.GROUPS` 한 벌이다 — 뷰는 `view-def` 에
    `"group"` 만 적고 이름·단계·주기는 페이로드(`d.groups`)로 받는다. 화면이 보내는
    `/api/run {groups}` 는 로컬·호스팅 둘 다 `run_all.group_names` 로 판정한다
