...
| `/capture gsc` | Search Console 실적 자동 수집 (구글 계정 연결 — 필수) |
...
| 원하는 것 | 키 없이 (기본) | 키를 쓰면 |
|---|---|---|
| 구글 실적 (필수 연결) | 내 구글 계정으로 **로그인 한 번**이면 소유한 모든 사이트가 붙습니다 (무료, 계정당 1회 5분 — 콘솔 클릭은 Claude가 대신) | — |
...
**구글 연결**은 내 구글 계정으로 **로그인 한 번**이면 끝입니다(계정당 한 번, 약 5분,
무료). 내가 소유한 속성이 한꺼번에 붙고, **속성마다 '사용자 및 권한'에 이메일을
추가하는 단계가 없습니다.** 채팅에 `GSC 연동해줘`라고 하면 구글 클라우드 콘솔
클릭(API 사용 설정 → OAuth 동의 화면 → 데스크톱 앱 클라이언트 만들기)은 Claude가
대신 해주고, JSON 다운로드 버튼만 직접 누르시면 됩니다 (브라우저 자동화가 크롬
다운로드를 트리거하지 못합니다 — 실측). 첫 조회 때 브라우저 로그인 창이 한 번
열립니다. 연결되면 번들된 **gsc MCP 서버**로 Claude가 서치콘솔을 즉석 조회할 수도
있습니다.

동의 화면을 테스트 모드로 두면 로그인 토큰이 7일마다 만료될 수 있습니다 — 그때는
다시 로그인 한 번이면 됩니다. 서버에서 사람 없이 돌려야 한다면 만료 없는 **서비스
계정** 갈래도 그대로 남아 있습니다(대신 속성마다 이메일 추가가 필요합니다).
절차는 `skills/setup/SKILL.md`의 "GSC 연결" 절에 있습니다.
...
├── gsc_oauth_client.json    # 구글 계정 로그인용 파일 (기본, 전 사이트 공용)
