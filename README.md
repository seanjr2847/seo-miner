...
- **`/capture run` 은 위 단계들을 순서대로 한 번에 돌리는 묶음**입니다. 일부만 돌리려면
  `--only` / `--skip` 을 씁니다. 단계 이름·순서의 정본은
  `skills/capture/scripts/run_all.py` 의 `STAGES` 이고, 그 목록은 아래
  [명령](#명령) 표에 한 벌만 적습니다.
- **`gsc` 만 체인을 멈춥니다.** 실패하면 남은 단계가 아예 안 돌고 요약표에 `안 돎` 으로
  찍힙니다. 다른 단계는 넘어져도 바퀴가 계속 돕니다.
...
- **`/capture run` 은 한 명령이 아니라 묶음입니다.** 여러 수집·분석 단계를 순서대로
  한 번에 돌립니다. 하나가 실패해도 나머지는 계속 가고, 유료 키가 없는 단계는 알아서
  빠집니다. 일부만 돌리려면 `--only gsc,gaps` 나 `--skip index,ai` 로 고릅니다.
  단계 이름·개수·순서의 정본은 `skills/capture/scripts/run_all.py` 의 `STAGES`
  하나입니다 — 여기에 옮겨 적지 않습니다.
- **`gsc` 가 실패하면 거기서 멈춥니다** — 나머지 전부가 그 데이터를 재료로 쓰기
  때문입니다. 다른 단계는 하나가 넘어져도 바퀴가 계속 돕니다.
...
| `/capture run` | **한 번에 끝까지** — `gsc → ga4 → index → keywords → metrics → rank → crawl → ai → competitors → backlinks → gaps → pages → vitals → report` 순서로 스크립트가 묶어서 돌고, 끝나면 리포트 파일을 알려 줍니다. 돈이 나가는 축(정본은 `run_all.py` `STAGES` 의 `is_paid`)은 키가 없으면 알아서 빠집니다 |
...
다른 단계는 하나가 실패해도 나머지가 계속 갑니다. 돈이 나가는 축(정본은 `run_all.py`
...
- 언어-지역은 사이트마다 고릅니다(목록 정본: `serp_adapter.LOCALES`). 그 나라 구글
  ...
  미국/영어로 떨어지며 경고만 냅니다.
- DataForSEO 유료 단계는 계정 한도(**분당 12회**, Live 엔드포인트)에 여유를 두고
  **분당 10회**로 호출 간격을 자동으로 벌립니다. 키워드 100개 순위 측정이 10분 남짓
  걸리는 이유입니다. 한도에 걸리면 기다렸다 다시 칩니다.
