...
`dashboard.ROUTES` 가 정답 모양이다 — 표를 데이터로 두고 두 서버가 같은 call 을 부른다.

> **정정 (09-11)**: 처음에 여기 "이주가 33개 라우트 중 6개에서 멈췄다" 고 적었는데
> 틀렸다. 33개 대부분은 원래 호스팅 전용이고(OAuth·사이트 등록·CLI 토큰·GA4 속성),
> 로컬 서버에는 호스팅에 절대 있으면 안 되는 로컬 전용 라우트(`/api/setup/*` — 로컬 키
> 저장·폴더·개발 도구 실행·원격 프록시)가 따로 있다. `ROUTES` 의 몇 개는 덜 끝난 이주가
> 아니라 **두 서버의 교집합**이다. 나머지를 표로 옮기지 마라.
>
> 같은 이유로 로컬 대시보드 서버를 FastAPI 로 바꾸지 않는다 —
> `docs/adr/0003-local-dashboard-stays-stdlib.md`.
...
## 8. 이 문서 이후 실제로 한 것 (2026-09-09)

문서가 계획만 담고 낡는 것을 막으려고 여기 붙인다. 정본은 커밋이다.

- **1·3·4·5·7번 전부 반영.** 세 갈래(수집 계약 / FastAPI / 정본·어휘·이스케이프)를
  각각 격리 worktree 에서 고쳐 합쳤다. `server/app.py` 1617 → 1116줄, 라우트 33개 중
  31개가 `Depends` 를 지난다. 전체 검사 41/43(실패 둘은 아래 "남은 빨강").
- **6번은 하지 않기로 한 대로 안 했다** — `assemble()` 은 그대로 두고 이스케이프만
  `htmlsafe.py` 로 좁혔다.
- **덤으로 고친 것**
  - `db.run_sql` 이 거절 경로(`sys.exit`)에서 커넥션을 안 닫던 것. 윈도우에서 임시
    brain 을 쓰는 검사가 정리 단계에서 죽었다.
  - `serp_adapter._dfs_call` 이 **429 만** 재시도하던 것 → 5xx 도 재시도한다
    (`SERVER_ERROR_RETRIES`). Fatal 로는 안 올린다.
  - `serp_adapter.fatal()` 이 402 를 무조건 "잔액 없음" 으로 부르던 것. 본문이 한도라고
    말하면 한도로 안내한다. 실제로 `잔액 없음(402) … [The rates limit per minute has
    been exceeded: 12 >= 12.]` 가 남아 있었다 — 원인은 한도인데 결제 화면으로 보냈다.

### 남은 빨강 두 개 (둘 다 기존, 기준선에서 동일하게 재현됨)

- `skills/setup/scripts/doctor.py --selfcheck` — 로컬 brain 상태에 따라 실패 지점이
  움직인다(상태 의존). `ddd2249` 에서도 같은 줄, 같은 메시지로 깨진다.
- `test_remote.py` — 호스팅 유료 단계의 `errors>0`. `metrics` 쪽은 v1.82.1 이 이미
  고쳤고 새 체인이 안 돌아 옛 행이 남아 있는 것뿐이다. `backlinks` 쪽은
  `domain_intersection` 이 9/6·9/8·9/9 세 번 다 500 이고, DataForSEO 는 본문에
  `Ok. / Internal Error.` 말고 아무 말도 안 한다. 우리 요청은 계약대로다
  (`targets` 는 20개 상한 dict).

### 아직 안 정한 것

재시도해도 `domain_intersection` 이 계속 500 이면, `test_remote` 가 유료 단계의
`errors>0` 을 무조건 FAIL 로 보므로 **게이트가 매 런 빨강으로 남는다**. 늘 빨간
게이트는 게이트가 아니게 된다(ADR 0002 가 막으려던 것). `collect_backlinks.py:312`
는 이미 "교집합은 … 없다고 단계를 죽이지는 않는다" 고 말하면서 500 일 때는 죽인다.
같은 논리를 5xx 에도 줄지는 **다음 런에서 재시도가 살리는지 보고** 정한다.
