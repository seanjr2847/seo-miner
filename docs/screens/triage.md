...
| 판정 되돌리기 | 행 오른쪽 [미판정으로 되돌리기] — **판정 거르개가 「미판정」이 아닐 때만** 보입니다 | 그 검색어가 다시 미판정으로 돌아오고, **무관이 껐던 키워드 추적도 함께 되살아납니다**(무관을 다른 판정으로 갈아탈 때도 같습니다). 되살리는 것은 **무관이 끈 줄만**입니다 — 애초에 후보라 꺼져 있던 줄과 사람이 직접 끈 줄은 그대로 둡니다 |
...
- **끈 것은 되돌릴 수 있습니다.** 무관을 거두면(되돌리기·보류·작업으로 갈아타기) 무관이 껐던 추적이 다시 켜집니다. 되살릴 줄은 "무관이 껐다"는 표식이 붙은 줄뿐입니다 — 꺼져 있다는 사실만으로는 애초에 후보였던 줄(적재 기본값이 꺼짐입니다)과 사람이 다른 이유로 끈 줄까지 켜 버리기 때문입니다. 웹 [키워드] 화면의 [추적에 넣기]·[추적에서 빼기]로 사람이 손대면 그 표식은 지워집니다 — 그 줄의 지금 상태를 정한 것은 이제 사람입니다. 이 표식이 생기기 전에 무관으로 꺼진 줄도 되살아나지 않습니다(안 켜는 쪽으로 틀리는 게 낫기 때문입니다).
...
판정 거르개를 「무관」으로 바꿉니다 → 그 줄의 [미판정으로 되돌리기] → 다시 미판정으로 돌아오니 [작업]으로 보냅니다. 무관이 껐던 키워드 추적은 되돌리는 그 순간 함께 켜지므로 따로 켤 것이 없습니다. (되돌리기를 거치지 않고 바로 [작업]·[보류]로 갈아타도 같습니다.)
...
flowchart TD
  ...
  D1 --> R4["다시 미판정으로 돌아오고<br/>무관이 껐던 추적도 되살아난다"]
```
...
<!-- 근거:
  ...
  skills/capture/scripts/db.py:550-557 (verdicts 표: project_id+key UNIQUE, id 로 pull/push 동행),
    1265 (VERDICTS = irrelevant·hold·work 정본),
    set_verdicts(None 이면 DELETE, irrelevant 는 is_active=1 인 줄만 is_active=0·verdict_off=1),
    _restore_verdict_off(무관을 거둘 때 verdict_off=1 인 줄만 되켠다 — 되돌리기·다른 판정 둘 다),
    keywords.verdict_off 열과 그 _migrate 분기(옛 Brain 의 기존 줄은 0),
    set_keywords_active(사람이 추적을 손대면 verdict_off=0),
    verdict_map, gate_sql(검색어 종류는 verdict='work' 여야 통과), open_opportunities gated=True
