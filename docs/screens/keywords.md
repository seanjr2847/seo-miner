...
| 추적할 검색어 켜고 끄기 *(웹 전용)* | [추적할 검색어]의 **후보 / 추적 중** 탭에서 체크 후 **[추적에 넣기]** / **[추적에서 빼기]** | 켠 검색어만 순위를 잽니다. 오른쪽에 `추적 중 N / M개`가 뜨고 한도에 닿으면 경고가 붙습니다. **여기서 사람이 손대면 [심사]의 「무관」이 껐다는 표식이 지워집니다** — 그 뒤로는 그 검색어의 무관 판정을 되돌려도 이 줄은 자동으로 다시 켜지지 않습니다(지금 상태를 정한 것이 사람이기 때문입니다) |
...
<!-- 근거:
  ...
  skills/capture/scripts/db.py — set_keywords_active(사람이 추적을 손대면 keywords.verdict_off=0),
    set_verdicts/_restore_verdict_off(「무관」이 껐던 줄만 되켠다 — [심사] 문서와 같은 한 벌)
-->
