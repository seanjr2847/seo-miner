...
이 화면에는 "기록 추가" 버튼도, "병합됨으로 바꾸기" 버튼도 **없습니다**. 읽기만 하는 자리입니다. 기록이 들어오는 길은 하나뿐입니다:
...
1. `createdb.py sync <사이트> --repo .` — 그 브랜치의 **머지된 PR**이 있는지 `gh` 로 확인해, 있으면 기록을 병합됨으로 켜고 그 기회를 완료로 닫습니다. `gh` 가 없거나 로그인이 안 됐으면 한 줄 알리고 건너뜁니다(실패로 치지 않습니다).
2. `createdb.py merged <기록번호>` — 손으로 하나 켭니다.

단, **호스팅이 가진 사이트는 기록의 병합됨을 켜는 서버 창구가 아직 없습니다.** 그래서 호스팅 기록은 PR이 머지돼도 '병합 전'으로 남고, 다음 `sync` 가 같은 PR을 다시 묻습니다(기회 쪽은 이미 완료로 닫혀 다시 닫지 않습니다). 호스팅 사이트에서 병합 전 건수가 안 줄어들면 이것이 원인입니다 — 실제로 안 고친 게 아닙니다.
...
| 실패한 실행을 다시 돌린다 | **문제로 판정된 줄**의 기록 칸에만 서는 칩을 누른다 | 로컬에서는 `/capture <단계> <사이트>` 명령이 복사된다. 호스팅에서는 버튼이 그 단계를 실제로 돌린다. 전체 실행(종류가 `전체`)이면 전체 분석 실행 쪽으로 간다. **박제본(읽기 전용)에는 안 선다** |
...
| 전부 다시 돌린다 | 맨 위 [다음 행동] 띠의 [전체 분석 실행] | 로컬은 `/capture run <사이트>` 복사, 호스팅은 전체 실행. **이 버튼은 화면에 하나뿐이다** — 실행 기록 머리에 같은 버튼을 또 두지 않는다 |
...
flowchart TD
  ...
  B -->|"14일 넘게 안 돈 종류가 있다"| D["전체 분석 실행"]
...
| 보이는 것 | 왜 | 어떻게 하나 |
| --- | --- | --- |
| "아직 실행한 적이 없습니다" | 이 사이트로 수집을 한 번도 안 돌렸다 | 맨 위 [전체 분석 실행]. 한 번 돌면 무엇을 언제 얼마에 수집했는지가 줄로 남는다 |
...
| 병합 전 건수가 `sync` 해도 안 줄어든다 | 호스팅이 가진 사이트다 — 기록의 병합됨을 켜는 서버 창구가 아직 없다 | 기회 쪽은 이미 완료로 닫혀 있다. 반영 여부는 PR 쪽에서 확인한다 |
...
<!-- 근거:
  ...
  기록을 남기는 개발 도구: skills/create/scripts/createdb.py
    ...
    - sync_merged 가 gh 로 머지된 PR 확인 → 로컬은 merged 켜고 기회 닫음, 호스팅은
      merged 창구가 없어 '병합 전'으로 남음: 215~274행
