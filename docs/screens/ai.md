...
쓰는지(인용) 이름만 부르는지(언급) 아무 데도 없는지를 세는 화면입니다. 같은 화면 아래
**[구글 AI 요약]** 칸(`#ai-aio`)은 구글 검색결과 맨 위에 뜨는 AI 요약을 봅니다 — 메뉴의
[AI 노출] 묶음이 "AI 가 우리를 싣나"를 묻는 한 자리라서 여기 둡니다. 둘은 재료가 다릅니다:
챗봇 칸은 검색 엔진을 거치지 않고 챗봇에 문장으로 물은 답변(AI 인용 확인 단계)이고, 구글
AI 요약 칸은 검색어 하나를 구글에 조회한 결과(순위 확인 단계가 같이 걷어 옴)입니다. 그래서
[이 묶음 다시 재기]는 두 단계(`ai`·`rank`)를 같이 돌립니다 — 묶음의 단계 목록 정본은
`run_all.GROUPS` 입니다.

**[구글 AI 요약] 칸** — 부제가 `AI 요약이 뜬 N개 중 M개에 내 페이지가 출처로 실렸습니다.` 로
지금 상태를 말하고(개수는 30개로 잘리기 전 전체 조회에서 셉니다), 본문 표가 요약은 떴는데
내 링크만 빠진 검색어를 `검색어` `순위` `대신 인용된 곳` `점수` 로 보여 줍니다(목록은 서버의
`aio_gap` 그대로). 줄을 누르면 그 검색어가 `구글 AI 요약 빠짐` 기회일 때 기회 패널이, 아니면
순위 구간에 맞는 서버 처방(`aio_play`)이 펼쳐집니다. 순위 조회가 한 번도 없거나 제공자가
요약을 주지 않으면(`serper`) 그렇다고 말하는 빈 상태가 섭니다 — 챗봇 확인이 없어도 이 칸은
섭니다.
