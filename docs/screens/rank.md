...
| AI 요약에 내 링크가 빠진 검색어를 **묶음으로** 본다 | 화면 맨 아래 [AI 요약] 칸의 **[남의 링크만 N개 보기]** 버튼 | 위 목록이 `남의 링크만`으로 걸러지고 화면이 목록으로 스크롤된다. 찾기 글자·지층 칩은 함께 풀린다 — 안 풀면 "N개 보기"라고 해 놓고 한 줄만 남는다 |
| 그중 **한 검색어만** 본다 | 같은 칸의 **검색어 칩**을 누른다 | 칩에 적힌 그 검색어를 위 목록에서 찾아 남긴다. 찾기 칸(`검색어로 찾기`)에 그 말이 들어가고, 지층 칩과 AI 요약 고르개는 함께 풀린다(안 풀면 빈 목록이 된다). 같은 칩을 다시 누르면 풀리고, 눌린 칩은 지층 칩과 같은 모양(`aria-pressed`)으로 선다 — 도구줄의 [조건 지우기]나 다른 칩으로 조건이 풀리면 이 칩도 같이 풀린다 |
...
**[AI 요약] 칸** — 부제가 `AI 요약이 뜬 N개 중 M개에 내 페이지가 출처로 실렸습니다.` 로 지금 상태를 말하고, 본문에는 누를 것만 둡니다: 나머지가 몇 개인지 한 줄 + **[남의 링크만 N개 보기]** 버튼(묶음으로 보는 길), 그 아래 내 링크가 빠진 검색어 칩 목록(`누르면 위 목록에서 그 검색어를 찾아 남깁니다. 다시 누르면 풀립니다.`).
...
맨 아래 [AI 요약] 칸에서 `AI 요약이 뜬 N개 중 M개에 내 페이지가 출처로 실렸습니다`를 읽습니다 → **[남의 링크만 N개 보기]** 를 누르면 위 목록이 그 행들만 남습니다 → 그중 하나가 궁금하면 아래 검색어 칩을 눌러 그 검색어만 남깁니다 → 줄을 펼치면 누가 대신 인용됐는지(인용 도메인)가 나오고, 우리 순위가 1페이지 안인지 밖인지에 따라 다른 처방이 붙습니다.
...
<!-- 근거:
  skills/capture/templates/views/rank.html — 이 화면 전부.
    ...
    RK_only/RK_setBand/RK_reset, RK_GAP(d.aio_gap 한 벌 — 칩은 검색어 글자가 아니라
    이 배열의 색인을 가리킨다), RK_gapChips()(눌린 칩은 .on·aria-pressed),
    RK_gapAll()(= [남의 링크만 N개 보기], RK_Q·RK_BAND 를 풀고 RK_AIO="gap"),
    RK_gapOnly(i)(= 검색어 칩, RK_Q 를 그 검색어로 두고 RK_BAND·RK_AIO 를 푼다·다시 누르면 해제),
    RK_paint() 끝의 `$("rk-gapchips").outerHTML = RK_gapChips()`(칩이 지금 조건을 따라간다),
