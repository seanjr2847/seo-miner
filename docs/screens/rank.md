# [순위 추적] — 추적 검색어가 지금 구글 결과에서 몇 위인지 보는 곳

추적 중인 검색어를 실제 구글 검색 결과에서 한 번 조회해, 검색어마다 지금 몇 위인지·직전 확인 대비 몇 칸 움직였는지를 한 화면에서 봅니다. 같은 조회가 걷어 오는 **구글 AI 요약**(요약이 떴는지·거기 내 링크가 실렸는지)은 이 화면이 아니라 [AI 인용] 화면의 [구글 AI 요약] 칸이 봅니다 — 메뉴에서 [AI 노출] 묶음이 사용자가 AI 요약을 찾는 자리라서입니다([AI 인용](ai.md) 참고).
...
| 기회 상태로 거른다 | [검색어별 기록] 도구줄의 기회 상태 고르개(기본 `전체` — 고를 수 있는 값의 정본은 셸의 `OPP_ST_OPT`, 개수가 같이 적힌다) | 목록이 그 상태의 기회가 붙은 검색어만 남는다 |
| 검색어로 찾는다 | 도구줄의 `검색어로 찾기` 칸에 글자를 친다 | 목록이 그 글자를 포함한 검색어만 남는다(칩·고르개와 함께 걸린다) |
| 조건을 지운다 | 걸러서 0건이 됐을 때 뜨는 [조건 지우기] | 찾기 글자·지층 칩이 모두 풀린다 |
| 한 검색어를 파고든다 (행 펼치기) | 목록의 줄을 누른다(키보드는 Enter·Space). 줄 앞 `▸`가 `▾`로 바뀐다 | 그 줄 밑이 열려 지금/직전 순위·SERP 피처, 순위에 걸린 페이지 URL, 그리고 할 일과 요청문이 나온다 |
...
**[검색어별 기록] 표** — 열은 `검색어` `점수` `순위` `이전` `Δ` `노출`. `이전`·`Δ`는 직전 확인이 있을 때만 생깁니다.
...
**줄을 펼치면** — 한 줄 요약(지금 N위 · 직전 N위 Δ · SERP 피처 이름), 순위에 걸린 페이지 URL, 그리고
...
[CSV 내려받기]는 **지금 걸러 놓은 목록 그대로** 내보냅니다(칩·고르개·찾기 칸이 걸린 뒤의 줄). 파일 이름은 `seo-miner-순위-<내려받은 날짜>.csv`, 엑셀에서 한글이 안 깨지게 BOM 이 붙습니다. 열은 이 여덟입니다:

`검색어`, `점수`, `종류`, `순위`, `이전`, `Δ`, `노출`, `구간`
...
이건 이 화면이 아니라 [AI 인용] 화면의 [구글 AI 요약] 칸이 답합니다([AI 인용](ai.md) 참고). 재료는 이 화면과 같은 순위 조회입니다.
...
**4. "순위 회의에 쓸 표가 필요하다"**
보고 싶은 구간 칩과 기회 상태 고르개를 걸어 목록을 좁힙니다 → [CSV 내려받기] → 점수·종류·순위·노출까지 들어 있어서 화면 밖에서도 왜 이 순서인지 읽힙니다.
...
[순위 다시 확인]. 다시 확인하면 검색어마다 지금 몇 위인지 잡힙니다(AI 요약은 [AI 인용] 화면에 섭니다). 비용 안내가 같은 문장에 붙습니다.

**"지난 조회에서 N건이 실패했습니다"**
실패 상자가 목록 위에 따로 섭니다. 다시 확인하면 실패한 검색어부터 이어서 채워집니다.

**"이 조건에 맞는 검색어가 없습니다"**
자료가 없는 게 아니라 거르개를 너무 좁게 건 것입니다. [조건 지우기]를 누르면 찾기·지층 칩이 모두 풀립니다.
...
**필요한 키**
순위 확인은 **유료 단계**입니다. DataForSEO(아이디·비밀번호) 또는 Serper(API 키) 중 하나가 있어야 돕니다. 없어도 기회 목록까지는 그대로 갑니다. Serper 는 더 싸지만 **구글 AI 요약은 못 봅니다**([AI 인용] 화면의 그 칸이 비게 됩니다). 호스팅에서는 키를 서버가 대므로 사용자가 넣을 것이 없습니다.
...
<!-- 근거:
  skills/capture/templates/views/rank.html — 이 화면 전부.
    ...
    RK_only/RK_setBand/RK_reset,
    RK_playParts()(281~340행: 순위 구간별 다섯 갈래 처방),
    ...
    RK_csv()(CSV 여덟 열), VIEW("rank")(502~709행: 부제 조건줄·
    빈 상태 세 갈래·errBox·계기판 다섯 칸·띠 문장·30개 컷 문구). 구글 AI 요약 칸은 views/ai.html 의 #ai-aio(AI_aio) 로 옮겼다,
