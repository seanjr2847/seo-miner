...
**1. 행동 줄 (맨 위)**
「요청문 만들기」 접이식이 주요 자리이고, 그 옆에 **지금 상태 칩**(「할 일」 「진행 중」 …),
그리고 지금 상태에서 누를 수 있는 나머지 상태 버튼이 섭니다. 여섯 화면에서는 여기에 다음
상태 버튼(「작업 시작」 등)과 실행 버튼(「Claude Code 로 열기」)도 함께 섭니다. [개요]는
그 검색어를 자세히 보는 원래 화면으로 가는 버튼을 하나 더 답니다. 줄에 설 것이 하나도
없으면(박제본 등) 줄 자체가 사라집니다.

지금 상태 칩이 여기 서는 이유는, 버튼은 **무엇으로 바꾸나**만 말하기 때문입니다
(「완료 표시」). [개요]는 접힌 줄이 「할 일」일 때 그 배지를 일부러 안 달아서, 기본
상태의 카드를 펼치면 지금이 어디인지 말해 주는 것이 하나도 없었습니다. 버튼이 아예 없는
**박제본·읽기 전용**에서도 이 칩은 남습니다 — 거기서는 이것이 유일한 상태 표시입니다.

「요청문 만들기」를 펴면 상자가 한 줄을 통째로 쓰고, 그 안에 들어 있는 것은 아래
**「요청문 상자」** 항목에 적었습니다.
...
위 **행동 줄(맨 위)** 에 접혀 있고, 펴면 그 요청문의 **꼴 이름**(있는 페이지 고치기 / 의도 갈라 내기 / 새 글 설계 /
...
<!-- 근거:
  skills/capture/templates/dashboard.html
    ...
    L2074-2092  oppActs(접힌 줄/행동 줄) · oppNow(지금 상태 칩 — READONLY·박제본에서도 남는다)
                · oppRest(펼침 패널 전용 — 「이 기회만 빼기」)
                · oppPanel(o, extra) — extra 가 그 화면만 아는 손잡이를 같은 행동 줄에 세운다
                  (화면이 패널 뒤에 제 상자를 또 달지 않게). .det-top:empty 면 줄이 사라진다
