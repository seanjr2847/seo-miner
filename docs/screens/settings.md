...
읽어 **빈 칸만** 미리 채웁니다. 이 자리는 **새로 등록하는 자리**라 이미 등록한 이름은
덮어쓰지 않고 **거절합니다** — 등록해 둔 값을 고치는 법은 「이름」 칸 밑 한 줄과 아래
[주의]에 적혀 있습니다.
...
자동화를 켜는 것이고, 없으면 그 단계만 건너뜁니다. 칸마다 아래에 **[저장된 키
지우기]** 체크칸이 붙어, 넣는 것뿐 아니라 **지우는 것도 여기서** 합니다(저장된 키가
하나도 없으면 지울 것이 없으므로 이 체크칸이 안 보입니다).
...
| 저장된 API 키 지우기 | 그 칸 아래 **[저장된 키 지우기]** 를 체크하고 [키 저장] | 체크한 칸은 입력이 막히고 비워집니다(한 칸이 "이 값으로 바꿔라"와 "지워라"를 동시에 뜻하지 않게). 저장을 누르면 **무엇을 지우는지 이름을 대고 한 번 더 묻고**, 확인해야 지웁니다. 끝나면 「키 N개를 지웠습니다」(값도 같이 넣었으면 「저장하고 키 N개를 지웠습니다」)가 뜹니다. **되돌릴 수 없습니다** — 다시 쓰려면 발급처에서 새로 받아 넣어야 합니다 |
...
- **키와 시크릿은 화면에 다시 뜨지 않습니다.** [키 저장]·[클라이언트 저장]을 누르면
  ...
  (OAuth 클라이언트)입니다. 화면에는 [N개 저장됨] 같은 개수만 남습니다. **칸이 비어
  보여도 저장된 키는 그대로 있습니다** — 몇 개가 저장돼 있는지는 오른쪽 배지가 말합니다.
- **4단계에서 빈 칸은 저장되지 않습니다.** 화면은 값이 적힌 칸과 **지우기로 찍은 칸만**
  보냅니다 — 그냥 비워 둔 칸은 안 보내므로 이전에 저장한 키가 그대로 남습니다. 한 칸만
  채워 저장해도 나머지 셋을 잃지 않는 이유가 이것입니다. 네 칸이 모두 비고 찍은 칸도
  없으면 "입력한 칸도, 지우기로 찍은 칸도 없습니다"로 끝납니다. **비우는 것으로는 못
  지웁니다 — 지우려면 [저장된 키 지우기]를 찍어야 합니다.** (사이트별 로컬 폴더는
  반대입니다 — 비우고 [저장]하면 그 줄이 지워집니다.)
- **이미 있는 이름으로는 등록되지 않습니다.** 2단계는 **새로 등록하는 자리**라 같은
  이름이면 "이미 있습니다 — 다른 이름을 쓰세요"로 **거절합니다**(덮어쓰지 않습니다).
  등록해 둔 값을 고치려면 `~/.capture/projects/<이름>.yaml` 을 고치고, 그 파일 맨 위
  줄이 시키는 `python db.py sync-project <yaml>` 을 한 번 돌려 반영합니다.
...
<!-- 근거:
  ...
  skills/capture/templates/dashboard.html — renderSetup/renderUsage/usageOpts(라디오 즉시
    ...
    KEYF·keyName(칸 이름의 정본은 뷰의 <label>)·keyDel 과 셸이 칸마다 다는 [저장된 키
    지우기] 체크칸(.delkey, 찍으면 input.disabled), btn-keys 가 값이 있는 칸과 찍은
    칸만 보내는 루프·지우기 확인 대화(window.confirm)·토스트 세 갈래,
    renderSetup 의 `del.parentNode.hidden = !d.nkeys`(저장된 키가 없으면 안 보임),
    btn-proj 성공 뒤 setTimeout 900ms → overview,
