...
| 할 일 | 어떻게 | 무엇이 바뀌나 |
| --- | --- | --- |
| 전부 다시 잰다 | 화면 머리줄의 [전체 다시 재기]를 누른다. 전체 재기 버튼은 이 화면 한 곳에만 섭니다(다른 화면 머리줄은 그 묶음만 다시 재는 [이 묶음 다시 재기]) | 잴 단계가 있는 묶음 전부가 한 런으로 돕니다 — 묶음끼리는 동시에, 순서가 필요한 단계만 기다립니다(묶음·순서의 정본은 `run_all.GROUPS`·`run_all.AFTER`). 도는 중이면 버튼이 그 상태를 말하고, 누르면 대기열에 올라 지금 런이 끝난 뒤 이어서 돕니다. **박제본에는 안 선다** |
...
<!-- 근거:
  ...
  skills/capture/templates/dashboard.html — renderGuide()(카드 done/here/later 클래스,
    ...
    SM.navDraw()(묶음은 view-def 의 group + 페이로드 d.groups — 정본 run_all.GROUPS), load()(사이트 이름이 없으면 settings 있으면 settings, 없으면 guide),
